...
void initWeapon() {
	...
	g.ammo = 30;
	g.isFired = false;
	g.gunTexture = loadTexture("textures/guns/pistol-1.png");
}

void drawWeapon(int num) {
	g.weapon = num;
	if (g.weapon == 1) { //pistol (default)
		g.gunTexture2 = loadTexture("textures/guns/pistol-2.png");
		g.gunTexture3 = loadTexture("textures/guns/pistol-3.png");
	...
	glColor3f(1, 1, 1);
	drawTexRect(g.gunTexture, 0, 1, 0, 1, g.gunX, g.gunX + g.gunW, g.gunY, g.gunY + g.gunH);
	sway();
}
...
void fireGun(float deltaTime) {
	...
	if (g.weapon == 1) {
		if (currentTime < 0.15) {
			g.gunTexture = g.gunTexture2;
			//printf("gun frame 2 \n");
		}
		else if (currentTime < 0.3) {
			g.gunTexture = g.gunTexture3;
			//printf("gun frame 3 \n");
		}
		else {
			g.gunTexture = loadTexture("textures/guns/pistol-1.png");
